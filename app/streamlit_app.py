...
if st.button("Compute risk"):
    # Build arrays dynamically from feature lists
    patient = {
    ...
    st.subheader("Contributions (approx.)")
    ...
    if st.button("Improve lifestyle"):
        new_Xd = np.array([[age, max(10,bmi-3), max(40,glucose-10), family_history, min(168,activity_hours+3)]])
